...
const getReverseDependencies = (body) => {
...
const getDownloadFilter = (body) => {
  return new Promise(function (resolve, reject) {
    const { snapdate, slideValue } = body;
      ...
      if (level === 4) {
        ...
        return;
            ...
            for (const version of crate["dates"]) {
              prev_date = Date.parse(version["created_range"][0]);
              future_date = Date.parse(version["created_range"][1]);
...
const getKeywordSnapshot = (body) => {
  return new Promise(function (resolve, reject) {
    const { snapdate, value } = body;
    console.log(snapdate);
    console.log(value);
    let curr_date = Date.parse(snapdate);
